...
void write_file_text( Alumns*, int );
char* formatter( Alumns*, int );
void printer( Alumns*, int );
...
void execute()
{
	...
	printer( alumns, array_count );

	write_file_text( alumns, array_count );
...
void write_file_text( Alumns *data_alums, int index_data )
{
	char *path_file_text = "/workspaces/cpp_devcontainer/programacion_1/m8/_files/data.txt";
	FILE *file_text;
	char *data_text;

	file_text = fopen( path_file_text, "w" );
...
void printer( Alumns *data, int array_count )
{
